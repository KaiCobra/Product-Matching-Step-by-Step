...
## clone
```bash
git clone -b Kai3 https://github.com/KaiCobra/Product-Matching-Step-by-Step.git
```
...
3. 使用 Product Matching Step-by-Step 做第二階段分類匹配。具體流程大致如下：
    1. 輸入兩個商品名稱作為待匹配的商品
    2. 使用設計好的 prompt template 及 RAG 語料庫，根據商品名稱，個別對兩個商品生成描述。
        具體 prompt 大致如下：
        ```
        ...
        ```
        產品名稱：
        不鏽鋼切糖機 送刀片 可調整厚度 切塊機 切片機 手工自製年糕 牛軋糖 Aaa1186
        ```
        ```
        生成描述：
        根据提供的商品名称“不锈钢切糖机 送刀片 可调整厚度 切块机 切片机 手工自制年糕 牛轧糖 Aaa1186”，我们可以识别出以下几个关键词：

        1. **不锈钢**：材料，表示该设备使用的是不锈钢材质。
        2. **切糖机**：功能，表示该设备主要用于切割糖类食品。
        3. **送刀片**：配件或功能，表示该设备配有可更换的刀片。
        4. **可调整厚度**：功能描述，表示用户可以根据需要调整切割的厚度。
        5. **切块机**：功能，表示该设备可以将糖类食品切成块状。
        6. **切片机**：功能，表示该设备可以将糖类食品切成片状。
        7. **手工自制年糕 牛轧糖**：产品类型，表示该设备适用于制作手工自制的年糕和牛轧糖。
        8. **Aaa1186**：可能为产品型号或编号。

        综合来看，这是一台多功能的食品加工设备，特别适合用于切割糖类食品（如年糕和牛轧糖），并且具有可调节切割厚度的功能。同时，它还配备有刀片，可以进行切块和切片操作。
        ```
    3. 使用設計好的 prompt template，讓 LLM 根據上述兩個商品的描述進行商品匹配並格式化輸出，最終得到二元分類結果。具體 prompt 大致如下：
        ```
        ###Product1
        ...(product1 p_name)...
        ###Description1
